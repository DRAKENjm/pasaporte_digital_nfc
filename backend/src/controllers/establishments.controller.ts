...
export const EstablishmentsController = {
  ...
  async detalle(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      sendResponse(res, 200, { ...est.rows[0], reglas: reglas.rows }, 'Detalle del establecimiento');
    } catch (error) {
      next(error);
    }
  },
  ...
  async crear(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (error.code === '23505') {
        return next(new ApiError(409, 'Ya existe un establecimiento con ese RUC'));
      }
      next(error);
    }
  },
  ...
  async actualizar(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      if (!req.user) throw new ApiError(401, 'No autenticado');
      const { id } = req.params;
      ...
      const result = await query(
        `UPDATE establecimientos SET
         ...
         WHERE id = $1
         RETURNING *`,
        [id, categoria_id, razon_social, direccion, estado]
      );
  ...
  /** Categorías */
  async categorias(_req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const result = await query(
  ...
  async listarReglas(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      const result = await query(
  ...
  async crearRegla(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      if (!req.user) throw new ApiError(401, 'No autenticado');
      const { id } = req.params; // establecimiento_id
      const { nombre_accion, valor_puntos_por_sello, limite_diario_por_usuario } = req.body;

      if (!nombre_accion) throw new ApiError(400, 'nombre_accion es obligatorio');
  ...
  async actualizarRegla(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const { reglaId } = req.params;
      const { nombre_accion, valor_puntos_por_sello, limite_diario_por_usuario, estado } = req.body;
      ...
      const result = await query(
        `UPDATE reglas_sellos SET
         ...
         WHERE id = $1
         RETURNING *`,
        [reglaId, nombre_accion, valor_puntos_por_sello, limite_diario_por_usuario, estado]
      );
  ...
  async agregarPersonal(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      if (!req.user) throw new ApiError(401, 'No autenticado');
      const { id } = req.params; // establecimiento_id
  ...
  async estadisticas(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
