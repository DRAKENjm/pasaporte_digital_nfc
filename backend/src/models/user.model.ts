...
export const UserModel = {
  ...
  async findById(id: string) {
    const res = await query(
      `SELECT u.id, u.email, u.nombres, u.apellidos, r.nombre as rol_nombre,
              u.total_sellos, u.puntos_globales, u.estado, u.created_at,
  ...
  async createUser(
    ...
    if (roleRes.rows.length === 0) {
      throw new Error(`El rol ${roleName} no existe. Ejecuta el seed de roles.`);
    }
  ...
  async updatePuntosYSellos(usuarioId: string, sellosDelta: number, puntosDelta: number) {
    const res = await query(
      `UPDATE usuarios
  ...
  async verifyEmail(id: string) {
    const res = await query(
      `UPDATE usuarios SET email_verificado = TRUE, estado = 'ACTIVO' WHERE id = $1 RETURNING id`,
      [id]
    );
    return res.rows[0];
  },
